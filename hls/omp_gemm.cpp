...
///////////////////
// Top IP Function: omp_gemm
// Accel. type hash: 6757388164
// Num. instances: 1
// Wrapper version: 13
///////////////////
...
constexpr unsigned int FPGA_GEMM_II = 2;
constexpr int ts = 256;
constexpr unsigned int FPGA_PWIDTH = 256;

void omp_gemm_moved(const float A[ts*ts], const float B[ts*ts], float C[ts*ts])
{
#pragma HLS inline
...
void omp_gemm_wrapper(hls::stream<ap_uint<64> >& mcxx_inPort, hls::stream<mcxx_outaxis>& mcxx_outPort, ap_uint<256>* mcxx_memport) {
...
#pragma HLS interface m_axi port=mcxx_memport
   ...
   if (mcxx_flags_2[4]) {
      ...
      #pragma HLS pipeline II=1
         ap_uint<256> __tmpBuffer;
         __tmpBuffer = *(mcxx_memport + mcxx_offset_2/sizeof(ap_uint<256>) + __i);
         for (int __j=0; __j <(sizeof(ap_uint<256>)/4); __j++) {
            __mcxx_cast<float> cast_tmp;
            cast_tmp.raw = __tmpBuffer((__j+1)*4*8-1,__j*4*8);
            C[__i*(sizeof(ap_uint<256>)/4)+__j] = cast_tmp.typed;
         }
      }
   }
   if (mcxx_flags_0[4]) {
   ...
   if (mcxx_flags_2[5]) {
      ...
      #pragma HLS pipeline II=1
         ...
         for (int __j=0; __j <(sizeof(ap_uint<256>)/4); __j++) {
            __mcxx_cast<float> cast_tmp;
            cast_tmp.typed = C[__i*(sizeof(ap_uint<256>)/4)+__j];
            __tmpBuffer((__j+1)*4*8-1,__j*4*8) = cast_tmp.raw;
         }
         *(mcxx_memport + mcxx_offset_2/sizeof(ap_uint<256>)+ __i) = __tmpBuffer;
      }
   }
   {
      #pragma HLS protocol fixed
